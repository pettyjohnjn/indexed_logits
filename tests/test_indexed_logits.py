...
try:
    from indexed_logits import (
        indexed_logits,
        indexed_logits_backward,
        indexed_logits_forward,
    )
...
def test_memory_efficiency():
    ...
    torch.cuda.reset_peak_memory_stats()
    out = indexed_logits_forward(H, W, idx)  # noqa: F841 - held alive to measure peak memory
    torch.cuda.synchronize()
...
def test_nondeterminism():
    ...
    check_close_v2(grad_W1, grad_W_ref, "grad_W (collision)", dtype=dtype, rtol=5e-2, atol=5e-2)
...
def test_noncontiguous_inputs():
    ...
    try:
        ...
        if "contiguous" in str(e).lower():
            print("    Result: Clear contiguity error (expected)")
        else:
